 ...
 * 死亡年龄类问法（死时几岁/死时多大…）必须放在 death_last_words 之前，
 * 避免「死时几岁」被「死时」二字抢归临终遗言类。
 */
const DEATH_PATTERNS: ReadonlyArray<{ kind: DeathIntent; re: RegExp }> = [
  ...
  // death_age 先于 death_last_words：死时几岁/死时多大 等问法须归死亡年龄，而非临终遗言
  { kind: 'death_age', re: /死的时候.*(多少岁|几岁|多大)|死时(多少岁|几岁|多大)|享年|卒年|去世时(多少岁|几岁|多大)|活了多少岁/ },
 ...
 * 卒年/享年/去世/活了多少岁 为 death_age 问法专用入口（不含「死」字，须先放行才能进模式表）。 */
const DEATH_GATE = /(死|亡|丧命|殒命|遇害|杀|被杀|斩|结局|遗言|遗诏|遗嘱|遗书|遗表|遗令|托孤|临终|驾崩|卒年|享年|去世|活了多少岁)/;
